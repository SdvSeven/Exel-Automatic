...
SQL_SYSTEM_PROMPT = {
    ...
    ),  ### Вот это уже хорошо, единственное что не он должен решать по поводу off-topic, по-хорошему это должен делать агент выше который ему дает указание
}
...
def answer_question_sql(
    ...
    )  ### Коротко он конечно поймет, но если написать 1-2 предложения или до 200 слов - будет лучше, еще лучше дать ему пример ответа, здесь мы не боимся уже галлюцинаций
